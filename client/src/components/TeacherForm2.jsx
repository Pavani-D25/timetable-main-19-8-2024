...
const TeacherForm2 = () => {
    ...
    const handleViewTimetable = async () => {
        ...
        setLoading(true);
        try {
            const response = await axios.get(`http://localhost:5000/timetableview/${facultyId}`);
            navigate('/timetable-display-view', { state: { timetable: response.data, facultyId } });
        } catch (error) {
            console.error('Error fetching timetable', error);
            setError('There was an error fetching the timetable. Please try again.');
        } finally {
